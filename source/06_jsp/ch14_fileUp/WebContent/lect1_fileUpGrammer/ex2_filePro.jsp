...
<body>
	<%
		...
		try{
			...
			while(paramNames.hasMoreElements()){
				...
				oriFilenames[idx] = multiRequest.getOriginalFileName(param);
			...
			if(file!=null){
				...
				try{
					...
					if(serverFile.exists()){ // file이 존재하면 복사
						...
						os = new FileOutputStream("D:\\JU\\source\\06_jsp\\ch14_fileUp\\WebContent\\fileUpFolder"+file);
	...
	<img src="<%=conPath%>/fileUpFolder/<%=filenames[2] %>" alt="0번 첨부 안 함">
	<h3>1번째 첨부</h3>
	<h3><%=path %>/<%=filenames[1] %></h3>
	<img src="<%=conPath%>/fileUpFolder/<%=filenames[1] %>" alt="1번 첨부 안 함">
	<h3>2번째 첨부</h3>
	<h3><%=path %>/<%=filenames[0] %></h3>
	<img src="<%=conPath%>/fileUpFolder/<%=filenames[0] %>" alt="2번 첨부 안 함">
	<h3>이름: <%=name %></h3>
</body>
</html>