...
<body>
	<table>
		...
		<tr>
			<c:forEach var="mem" items="${members }"> <!-- session에 있는 변수 이름과 동일하면 안 됨 -->
				<td>
					<img src="${conPath }/memberPhotoUp/${mem.mphoto }" alt="${mem.mname }사진" width="120">
					<p>
						${mem.mname }<br>
						(${mem.mid })
					</p>
	...
	<p class="paging">
		...
		&nbsp; &nbsp; &nbsp;
		<c:if test="${pageNum > 1}">
			<a href="${conPath }/allView.do?pageNum=${pageNum-1}">&lt;</a>
		</c:if>
		<c:if test="${pageNum == 1}">
			&lt;
		</c:if>
