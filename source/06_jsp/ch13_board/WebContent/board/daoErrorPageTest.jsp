...
<body>
	...
	<%
		...
		out.println("<h3>3. 원글 쓰기 </h3>");
		BoardDto dto = new BoardDto(0, "김구", "대한만세", "제곧내", null, 0, "111", 0, 0, 0, "192.192.2.01", null);
		int result = bDao.insertBoard(dto);
		out.println(result == BoardDao.SUCCESS ? "글쓰기 성공":"글쓰기 실패");
		out.println("<h3>4 & 5. 조회수 올리고 글 번호로 dto 가져오기 </h3>");
		bDao.readCountUp(3);
		dto = bDao.getBoardOneLine(3);
		out.println("수정 전의 3번 글 : " +dto);
		out.println("<h3>6. 글 수정 </h3>");
		dto = new BoardDto(3, "김구", "수정제목", "수정내용", "modify@m.com", 0, "111", 0, 0, 0, "192.192.2.31", null);
		...
		out.println("<h3>7. 글 삭제</h3>");
		result = bDao.deleteBoard(5, "111");
		out.println(result == BoardDao.SUCCESS? "5번글 삭제 성공":"5번글 삭제 실패(비번)");
	%>
</body>
</html>