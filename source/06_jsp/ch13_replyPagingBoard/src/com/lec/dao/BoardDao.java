...
public class BoardDao {
	...
	public ArrayList<BoardDto> listBoard(int startRow, int endRow){
		...
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT * FROM (SELECT ROWNUM RN, A. * FROM (SELECT * FROM BOARD ORDER BY REF DESC, RE_STEP ASC) A) WHERE RN BETWEEN ? AND ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	private void preReplyStep(int ref, int re_step) {
		...
		try {
			...
			System.out.println("답변글 쓰기 위해 "+result + "개 행 업데이트");
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
			try {
	...
	public int reply(BoardDto dto) {
		...
		// 원글에 대한 정보, num, ref, re_step, re_level
		// 사용자로부터 입력 받는 내용: writer, subject, content, email, pw
		preReplyStep(dto.getRef(), dto.getRe_step());
		int result = FAIL;
		Connection			 conn = null;
		PreparedStatement	 pstmt = null;
		String sql = "INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)" + 
		...
		try {
			...
			result = pstmt.executeUpdate();
			System.out.println(result == SUCCESS ? "답변글 작성 완료" : "답변글 작성 실패");
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
			try {
