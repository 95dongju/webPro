...
UPDATE CUSTOMER SET cPW='111',
                ...
                WHERE cID='aaa';
-- 5. 회원 리스트
SELECT * FROM(SELECT ROWNUM RN, A. * FROM (SELECT * FROM CUSTOMER ORDER BY cRDATE DESC) A) WHERE RN BETWEEN 1 AND 3;
-- 6. 회원 수
...
INSERT INTO FILEBOARD (fNUM, cID, fSUBJECT, fCONTENT, fFILENAME, fPW, fREF, fRE_STEP, fRE_LEVEL, fIP)
    VALUES (fNUM_SEQ.NEXTVAL, 'aaa', '두번째님', '초코', NULL, '111', 2, 1, 1, '192.1.1.2');
-- 4. 글 수정
