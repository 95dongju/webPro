...
<body>
	...
	<div id="content_form" style="padding: 40px;">
		<%
			//fboardWriteForm.jsp => 원글 쓰기 (fsubject, fcontent, ffilename, fpw(사용자)) (fref, fre_step, fre_level (0))
			
			//fboardWriteForm.jsp?fnum=1&pageNum=9 => 1번글에 대한 답글 쓰기 (fsubject, fcontent, ffilename, fpw(사용자)) (fref, fre_step, fre_level (원글))
			...
			int fnum = 0;
			FileboardDto fDto = new FileboardDto(); // 빈 dto 객체 생성 (fnum, fref, fre_step, fre_level은 0으로 초기화)
			if(request.getParameter("fnum")!=null){ // fnum번 글 답글
				fnum = Integer.parseInt(request.getParameter("fnum"));
				FileboardDao fDao = FileboardDao.getInstance();
				fDto = fDao.getFileboardOneLine(fnum); // 원글에 대한 정보 (fref, fre_step, fre_level)
			}
		%>
						...
						<input type="text" name="fsubject" required="required" autofocus="autofocus" 
							value="<%
									if(fDto.getFsubject()!=null){
										out.println("[답변]" + fDto.getFsubject());
									}
								%>">
					</td>
				</tr>
