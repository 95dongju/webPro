...
import com.lect.dto.CustomerDto;

public class CustomerDao {
	public static final int SUCCESS = 1;
	public static final int FAIL = 0;
	public static final int CUS_EXISTENT = 0;
	public static final int CUS_NONEXISTENT = 1;
	...
	public int joinCustomer(CustomerDto dto) {
		...
		try {
			...
			System.out.println(e.getMessage());
			System.out.println("회원가입 예외 발생 :" +dto);
		} finally {
			try {
				if(pstmt!=null) pstmt.close();
				if(conn!=null) conn.close();
	...
	public ArrayList<CustomerDto> customerList(int startRow, int endRow){
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT * FROM(SELECT ROWNUM RN, A. * FROM (SELECT * FROM CUSTOMER ORDER BY cRDATE DESC) A) WHERE RN BETWEEN ? AND ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
