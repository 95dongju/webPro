...
public class FileUpService {
	...
	public boolean fileup(MultipartHttpServletRequest mRequest, ModelAndView mav) {
		...
		String backupPath = "C:\\JU\\source\\09_Spring\\ch13_fileupload\\src\\main\\webapp\\upload\\";
		...
		while(params.hasNext()) {
			...
			System.out.println(originalFileName==null ? "null": originalFileName.contentEquals("") ? "빈 string" : "첨부한 파일은 " +originalFileName);
			// 첨부를 안 하면 빈스트링
			if(fileNames[i]!=null && !fileNames[i].equals("")) {
				// 파일 첨부함
				if(new File(uploadPath + fileNames[i]).exists()) {
					// 첨부파일과 같은 이름의 파일이 서버에 존재하는 경우 160213554100.jpg
