...
import com.lec.ch11.service.BoardService;
...
public class MvcBoardController {
	...
	public String write(int bid, Model model) {
	...
	public String write_content(int bid, Model model) {
		model.addAttribute("bid", bid);
		bservice = new BContentService();
		bservice.execute(model);
		return "mvcBoard/content";
	...
	public String reply(@ModelAttribute("bDto") BoardDto boardDto, HttpServletRequest request, Model model) {
		model.addAttribute("request", request);
