	...
	<script>
		$(document).ready(function(){
			$('form').submit(function(){
				let confirmMsg = $('.confirmMsg').text().trim();
				if(!confirmMsg){
					alert('사번 중복체크를 하세요');
					frm.empno.focus();
					return false; // submit 이벤트 제한
				}else if (confirmMsg != '사용 가능한 사번입니다'){
					alert('중복된 사번은 사용할 수 없습니다');
					frm.empno.focus();
					return false;
				}
			});
...
<body>
	...
	<input type="hidden" name="pageNum" value="1">
		<table>
			...
			<tr>
				<th>이름</th><td><input type="text" name="ename" value="${param.ename }"></td>
			</tr>
			<tr>
				<th>직책</th><td><input type="text" name="job" value="${param.job }"></td>
			</tr>
			<tr>
				<th>상사</th>
				<td>
					<select name="mgr">
						...
						<c:forEach var="mgrEmp" items="${managerList }">
							...
							<c:if test="${mgrEmp.empno eq param.empno}">
								selected="selected"
							</c:if>
							>
