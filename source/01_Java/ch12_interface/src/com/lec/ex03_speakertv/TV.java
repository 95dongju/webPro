...
public class TV implements IVolume {
	...
	public void volumeUp() { // 볼륨 1씩 증가
		if (volumeLevel < MAX_VOLUME) {
			volumeLevel++;
			System.out.println("TV 볼륨을 1 올려 현재 볼륨은 " +volumeLevel);
		}else {
	...
	public void volumeUp(int level) {
		if (volumeLevel+level < MAX_VOLUME) {
			volumeLevel += level;
			System.out.println("TV 볼륨을 "+level+" 올려 현재 볼륨은 " +volumeLevel);
		}else {
			int tempLevel = MAX_VOLUME - volumeLevel;
			volumeLevel = MAX_VOLUME;
			System.out.println("TV 볼륨을 "+level+" 올려 최대치입니다");
		}
	}
	...
	public void volumeDown() {
		if (volumeLevel > MIN_VOLUME) {
			volumeLevel--;
			System.out.println("TV 볼륨을 1 내려 현재 볼륨은 " +volumeLevel);
		}else {
	...
	public void volumeDown(int level) {
		if (volumeLevel-level > MIN_VOLUME) {
			volumeLevel -= level;
			System.out.println("TV 볼륨을 "+level+" 내려 현재 볼륨은 " +volumeLevel);
		}else {
			int tempLevel = volumeLevel - MIN_VOLUME;
			volumeLevel = MIN_VOLUME;
			System.out.println("TV 볼륨을 "+level+" 올려 최소치입니다");
		}
	}
}
