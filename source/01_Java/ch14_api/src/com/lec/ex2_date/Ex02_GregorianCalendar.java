...
public class Ex02_GregorianCalendar {
	public static void main(String[] args) {
		//Calendar now = Calendar.getInstance(); // Calendar 은 싱글톤, 생성자 protected
		// Calendar는 싱글톤이기 때문에 시스템 내의 시간을 가지고 옴 GregorianCalendar는 생성자 함수이기 때문에 특정한 시간을 가지고 옴
		GregorianCalendar now = new GregorianCalendar(); //매개변수에 아무것도 넣지 않으면 '지금'임
		//Calendar와 GregorianCalendar는 똑같지만 타입이 다름, 
		System.out.println(now);
		int year = now.get(Calendar.YEAR);
		int month = now.get(Calendar.MONTH)+1;// 시스템은 0월부터 시작하기 때문에 반드시 +1을 해줘야 함
