  ...
  getContent(){
    ...
    if(this.state.mode === 'welcome'){
      ...
      _article = <CreateContent onChangePage={function(_title, _desc){
        // alert(_title+ '/' + _desc);
        // this.state.contents 에 {id:max_content_id+1, title:_title, desc:_desc}를 push
        this.max_content_id ++;
        
        // this.state.content.push(  --> state의 값을 변경하려면 setState() 함수 이용
        //   {id:this.max_content_id, title:_title, desc:_desc}
        // );

        // 방법 1.
        //var _contents = [ ... this.state.contents]; // 깊은 복사로 _content에 넣기
      ...
      _article = <UpdateContent data={_contents} onChangePage={function(_id, _title, _desc){
        // id가 _id인 contents 안 객체값을 교체함 {id: _id, title: _titld, desc: _desc}
    ...
    return(
      <div>
        ...
        <Control onChangePage={function(_mode){
          if(_mode === 'delete'){
            if(window.confirm('삭제하시면 복구 불가합니다. 삭제할까요?')){
              // this.state.selected_content_id가 id인 contents 안 객체를 제거
              var _contents = Array.from(this.state.contents);
              for(var idx=0; idx<_contents.length; idx++){
                if(_contents[idx].id === this.state.selected_content_id){
                  ...
                  var first; // id를 기준으로 오름차순 정렬후 첫번째 객체의 id값
